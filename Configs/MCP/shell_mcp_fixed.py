...
Fixes vs original shell_mcp.py:
  1. run_command: exit_code now preserves 0 (was `proc.returncode or -1`
     which turned a successful exit 0 into -1).
  2. _reader_loop: event-driven via loop.add_reader instead of busy-poll
     with sleep; buffer access protected by lock to avoid data races.
  3. run_command: timeout=None now falls back to DEFAULT_TIMEOUT (30s)
     instead of blocking forever, preventing event-loop deadlock.
  4. start_background: new tool for long-running/interactive commands
     (ssh -R, reverse shells, etc.) that returns immediately with a
     session_id; use send_input/read_output/close_session to interact.
"""
...
if __name__ == "__main__":
    log.info(f"Starting MCP Shell server (fixed) on {LISTEN_HOST}:{LISTEN_PORT}")
    print(f"MCP Shell server (fixed) starting on {LISTEN_HOST}:{LISTEN_PORT}")
